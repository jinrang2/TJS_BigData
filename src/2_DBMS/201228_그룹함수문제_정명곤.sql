...
--12. 위의 11번을 수정하여, 부서별 급여평균 최대치, 부서별 급여합의 최대치, 
            --부서별 최소급여의 최대치를 출력하라.
SELECT MAX(AVGSAL), MAX(MAXSAL), MAX(MINSAL)
FROM
(
SELECT DEPTNO, ROUND(AVG(SAL)) AVGSAL, MAX(SAL) MAXSAL, MIN(SAL) MINSAL
  FROM EMP
 GROUP BY DEPTNO
);
...
SELECT COUNT(*) AS TOTAL, SUM(DECODE(TO_CHAR(HIREDATE,'YYYY'),'1980',1)) AS "1980", 
...
SELECT JOB, SUM(DECODE(DEPTNO, 10, SAL,0)) AS DETPNO10,
       SUM(DECODE(DEPTNO, 20, SAL,0)) AS DEPTNO20,
       SUM(DECODE(DEPTNO, 30, SAL,0)) AS DPETNO30,
       SUM(SAL) AS TOTAL 
  FROM EMP
 GROUP BY JOB
 ORDER BY 1;
