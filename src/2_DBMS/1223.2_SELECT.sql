...
SELECT ENAME || '은 ' || JOB || ' 업무이고 연봉은 '|| TO_CHAR(SAL*12+NVL(COMM,0)) || '다' as DES FROM EMP;
...
SELECT * FROM EMP WHERE ENAME LIKE '__M__';

SELECT * FROM EMP WHERE ENAME LIKE '%M%';

SELECT * FROM EMP WHERE ENAME LIKE '%S';

DELETE FROM EMP WHERE ENAME LIKE '%\%%' ESCAPE '\';

SELECT A.* , SUBSTR(HIREDATE,4,2) AS MON  FROM EMP A ORDER BY MON;
...
SELECT * 
  FROM EMP 
 WHERE EXISTS (SELECT 1 FROM EMP A WHERE A.COMM = COMM);
  
 SELECT * FROM EMP WHERE COMM>0;
 
SELECT * FROM EMP ORDER BY ROWNUM;

SELECT ENAME , SAL*12+NVL(COMM,0) AS YEARCAL FROM EMP;

SELECT ENAME , SAL*12+NVL(COMM,0) AS YEARCAL 
  FROM EMP
...
SELECT ENAME, JOB, HIREDATE FROM EMP WHERE HIREDATE > TO_DATE('19810220','YYYYMMDD') AND HIREDATE < TO_DATE('19810501','YYYYMMDD') ORDER BY HIREDATE;
...
--출력하는 SELECT 문을 작성
SELECT ENAME, SAL, COMM FROM EMP WHERE COMM IS NOT NULL AND SAL*1.1 < COMM;
...
SELECT EMPNO, ENAME, SAL*12+NVL(COMM,0) AS YEARSAL FROM EMP WHERE SAL*12+NVL(COMM,0) > 35000;
