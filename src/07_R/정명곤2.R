#다음에 제시된 문제들을 R 코드로 작성한 후에 본인이름.R 로 저장하여 강사 메일로 전송합니다.
...
m1 = matrix(seq(10,38,2), ncol=5, nrow=3, byrow=TRUE)
m1
...
row_max = max(matrix(m1,nrow=1))
row_max = apply(m1,1,max)
row_max


# m1 에서 열 단위의 최대값을 추출하여 col_max 에 저장한다.
col_max = max(matrix(m1,ncol=1))
...
temp <- paste(count,week.korname, sep=':')
...
m3 <- matrix(m2, ncol=3, dimnames = list(c('row1','row2','row3'),c('col1','col2','col3')))
m3

# 	 
# [문제9] 다음과 같이 구성 되는 2행 3열 매트릭스 alpha를 생성한 후에
alpha <- matrix(c('a','b','c','d','e','f'), ncol=3)
alpha2 <- rbind (alpha , matrix(c('x','y','z'), ncol=3))
alpha3 <- cbind (alpha, matrix(c('s','p'),ncol=1))
...
abc <- rename(abc, c("a"="제품명"))
abc <- rename(abc, c("b"="가격"))
abc <- rename(abc, c("c"="판매량"))
abc
...
cat('과일가격평균 : ',mean(abc[,2]), ' 과일판매량평균 : ',mean(abc[,3]))
...
grade <- for (val in score){
  cat(val,':',switch(as.integer(val/50),'C','B','A'),'\n')
}
